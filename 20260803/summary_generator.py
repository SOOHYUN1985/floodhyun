...
def _calc_data_driven_targets(df: pd.DataFrame, current_price: float) -> Optional[Dict]:
    ...
    # 유사 상황 필터 (하락장에서 이격도가 100 미만이어도 정상 동작하도록 절대 하한 없이 상대 범위만 사용)
    disp_lo = cur_disparity - 5
...
def _build_market_block(market_name: str, current_price: float,
                        ...
                        strategies: List[Dict], df: pd.DataFrame,
                        ensemble_results: Dict = None,
                        bull_strategies: List[Dict] = None) -> str:
    """한 시장(코스피 or 코스닥)의 요약 블록 생성"""
    heat_score, hot_inds = _calc_overheat(df)
    bull_strategies = bull_strategies or []
    ...
    if triggered:
        ...
        # 선정된 상향돌파(breakout) 전략이 하나도 없는 경우
        # → 과거 유사 상황(이격도+60일 상승률) 기반 데이터 주도 목표가로 대체
        _next_targets = _calc_data_driven_targets(df, current_price)
        if _next_targets and _next_targets['targets']:
            lines.append('| 단계 | 목표가 | 등락률 | 근거 | 확률 |')
            lines.append('|:---:|---:|:---:|:---|:---:|')
            for j, tgt in enumerate(_next_targets['targets'], 1):
                lines.append(f'| ▶ {j}단계 | **{tgt["price"]:,.0f}** | {tgt["pct"]:+.1f}% | {tgt["label"]} | {tgt["prob"]}% |')
            lines.append('')
            lines.append('> 💡 상향돌파 전략 미선정(하락장 특성상 반전·손절 전략이 우선 선정됨) '
                          '→ 과거 유사 국면의 실제 상승폭 기반 참고 목표')
        else:
    ...
    if reversal:
        ...
        if len(reversal) >= 2:
            # 앙상블 투표 정보 추가
            if ensemble_results and ensemble_results.get('stats'):
                best_n = ensemble_results['best_n']
                best_wr = ensemble_results['best_win_rate']
                # N은 전체 전략 기준, reversal만이 아님
                effective_n = min(best_n, len(reversal))
                lines.append(f'| 강력 매도 | 전체 {best_n}개+ 전략 동시 발동 시 → 50% 청산 (검증승률 {best_wr:.0f}%) |')
            else:
    ...
    # ── 방향성 종합 판단 (매도 vs 매수) ──
    if strategies or bull_strategies:
        sell_n = len(strategies)
        buy_n = len(bull_strategies)
        def _avg(items, key='win_rate'):
            vals = [s.get(key, 0) for s in items if s.get(key) is not None]
            return sum(vals) / len(vals) if vals else 0
        sell_wr = _avg(strategies)
        buy_wr = _avg(bull_strategies)
        sell_avg = _avg(strategies, 'avg_return')
        buy_avg = _avg(bull_strategies, 'avg_return')
        ...
        if sell_imminent > 0 and sell_strength > buy_strength * 1.5:
            verdict = f"🔴 **하락 우세** ({sell_imminent}개 임박)"
        ...
        lines.append(f'**🎯 방향성 종합**: {verdict}')
        lines.append('')
        lines.append('| 방향 | 전략 | 평균 승률 | 예상 변동 |')
        lines.append('|:----:|:---:|:--------:|:--------:|')
        lines.append(f'| 🔴 하락(매도) | {sell_n}개 | {sell_wr:.1f}% | {sell_avg:+.2f}% |')
        lines.append(f'| 🟢 상승(매수) | {buy_n}개 | {buy_wr:.1f}% | {buy_avg:+.2f}% |')
        lines.append('')

        # 상위 1개 매도/매수 전략의 분포 미리보기
        if strategies:
            top = strategies[0]
            d = top.get('return_distribution', {})
            if d:
                lines.append(f'**📐 매도 Top1 [{top["name"]}] 20일 수익률 분포**')
                lines.append('')
                lines.append(f'  • 중앙값: **{d.get("median", 0):+.1f}%** | 5%분위 {d.get("p5",0):+.1f}% ~ 95%분위 {d.get("p95",0):+.1f}%')
                lines.append('')
        if bull_strategies:
            top = bull_strategies[0]
            d = top.get('return_distribution', {})
            if d:
                lines.append(f'**📐 매수 Top1 [{top["name"]}] 20일 수익률 분포**')
                lines.append('')
                lines.append(f'  • 중앙값: **{d.get("median", 0):+.1f}%** | 5%분위 {d.get("p5",0):+.1f}% ~ 95%분위 {d.get("p95",0):+.1f}%')
                lines.append('')

    # ── 리스크 조정 비중 (Half-Kelly Aggregate) ──
    kelly_strats = [s for s in strategies if s.get('half_kelly') is not None]
    if kelly_strats:
        # 상위 5개 전략의 평균 Half-Kelly
        top5 = kelly_strats[:5]
        avg_half_k = sum(s.get('half_kelly', 0) for s in top5) / len(top5)
        max_half_k = max(s.get('half_kelly', 0) for s in top5)
        # 신호 임박도가 높을수록 비중 줄임
        recommended = avg_half_k * 100  # %로 표시
        lines.append('**💰 Kelly 권장 매도비중**')
        lines.append('')
        lines.append('| 지표 | 값 | 의미 |')
        lines.append('|:---|---:|:---|')
        lines.append(f'| 평균 Half-Kelly (Top5) | **{recommended:.0f}%** | 매수 후 매도 신호 시 권장 청산비율 |')
        lines.append(f'| 최대 Half-Kelly | {max_half_k*100:.0f}% | 가장 강한 전략의 권장 비중 |')
        # 평균 Sharpe
        sharpes = [s.get('sharpe_ratio', 0) for s in top5 if s.get('sharpe_ratio') is not None]
        if sharpes:
            avg_sh = sum(sharpes) / len(sharpes)
            sh_grade = '우수' if avg_sh >= 1.0 else ('보통' if avg_sh >= 0.5 else '약함')
            lines.append(f'| 평균 Sharpe (Top5) | {avg_sh:.2f} | {sh_grade} |')
        # 신호 임박 카운트
        impending = 0
        for s in breakout:
            tp = _calc_trigger_price(s, df)
            if tp is not None:
                ...
                if 0 < gap <= 2:
                    impending += 1
        lines.append(f'| 2% 이내 임박 전략 수 | {impending}개 | 신호 발동 임박도 |')
        lines.append('')
...
def _build_conditional_mdd_block(df: pd.DataFrame, market_name: str, current_price: float) -> str:
    ...
    # 하락장에서 이격도가 100 미만이어도 정상 동작하도록 절대 하한 없이 상대 범위만 사용
    disp_lo = cur_disparity - 5
    disp_hi = cur_disparity + 5
    rally_lo = cur_rally - 10
    rally_hi = cur_rally + 10
...
def generate_summary(market_results: List[Dict]) -> str:
    ...
    for mr in market_results:
        block = _build_market_block(
            ...
            df=mr['df'],
            ensemble_results=mr.get('ensemble_results'),
            bull_strategies=mr.get('bull_strategies', []),
        )
        lines.append(block)
        lines.append('---')
        lines.append('')
