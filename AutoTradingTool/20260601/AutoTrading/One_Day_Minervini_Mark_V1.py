...
def One_Day_Minervini_Mark_V1(profit_loss_ratio, data, logger, logLevel, strategy, jisu_data, index_data):
    ...
    ma5_jisu_data = jisu_data[f'MA_{20}'].tolist()
    ...
    for i in range(59, len(data)):
        ...
        ma33_jisu = ma20_jisu_data[jisu_index]
        ...
        conditions.append(lambda: close_jisu > close_jisu_data[i-1]*0.96)                              # 지수가 +인경우
        ...
        if buy_condition and open_val != high != close:
            ...
            recent_299_days_high_price = high_data[i-298:i]    # 최근 50일간의 가격 데이터
            max_close_price = max(recent_200_days_close_price)         # 최근 50일간의 최고 가격
            max_high_price = max(recent_299_days_high_price)         # 최근 50일간의 최고 가격
            long_conditions = []
            ...
            long_conditions.append(lambda: data['VOLUME_5'].iloc[i] > data['VOLUME_5'].iloc[i-20])      # 5일 거래량 이평선이 20일전대비 감소
            long_conditions.append(lambda: data['VOLUME_50'].iloc[i-1] >= volume_data[i-1] or data['VOLUME_50'].iloc[i-2] >= volume_data[i-2] or data['VOLUME_50'].iloc[i-3] >= volume_data[i-3])
            ...
            if buy_upper_day_bound <     buy_after_day and sell_price <     buy_price :
                sell_signals.append(data['date'].iloc[i])
                sell_qty = remaining_qty
                remaining_qty = 0
                profit = (sell_price - buy_price) / buy_price * 100
                buget = buget + sell_qty * sell_price
            ...
            for j in range(nTime):
                ...
                if close <  data[f'EMA_{strategy[1][j]}'].iloc[i]and sell_list[j]== False and buy_price <   sell_price :
                    sell_list[j] = True
