    ...
    public class DbManager
    {
        ...
        public void EnsureTables()
        {
            using (var conn = new SQLiteConnection(_dbPath))
            {
                ...
                // 일별 자산 추이 테이블 (날짜 PK — 하루 1건)
                string createDailyAsset =
                    "CREATE TABLE IF NOT EXISTS DailyAssetTable (" +
                    "날짜 TEXT PRIMARY KEY, 추정예탁자산 INTEGER, 총매입금액 INTEGER, 총평가금액 INTEGER, " +
                    "보유평가손익 INTEGER, 당일실현손익 INTEGER, 보유종목수 INTEGER, " +
                    "당일매수건수 INTEGER, 당일매도건수 INTEGER, 당일매도승수 INTEGER, 당일매도패수 INTEGER)";
                using (var cmd = new SQLiteCommand(createDailyAsset, conn))
                    cmd.ExecuteNonQuery();
            }
        }
        ...
        /// <summary>
        /// 일별 자산 추이 저장 (날짜 기준 UPSERT — 같은 날 재실행 시 덮어쓰기)
        /// </summary>
        public void UpsertDailyAsset(DailyAssetRecord record)
        {
            using (var conn = new SQLiteConnection(_dbPath))
            {
                conn.Open();
                string sql =
                    "INSERT OR REPLACE INTO DailyAssetTable " +
                    "(날짜, 추정예탁자산, 총매입금액, 총평가금액, 보유평가손익, 당일실현손익, " +
                    "보유종목수, 당일매수건수, 당일매도건수, 당일매도승수, 당일매도패수) " +
                    "VALUES (@날짜, @추정예탁자산, @총매입금액, @총평가금액, @보유평가손익, @당일실현손익, " +
                    "@보유종목수, @당일매수건수, @당일매도건수, @당일매도승수, @당일매도패수)";
                using (var cmd = new SQLiteCommand(sql, conn))
                {
                    cmd.Parameters.AddWithValue("@날짜", record.날짜);
                    cmd.Parameters.AddWithValue("@추정예탁자산", record.추정예탁자산);
                    cmd.Parameters.AddWithValue("@총매입금액", record.총매입금액);
                    cmd.Parameters.AddWithValue("@총평가금액", record.총평가금액);
                    cmd.Parameters.AddWithValue("@보유평가손익", record.보유평가손익);
                    cmd.Parameters.AddWithValue("@당일실현손익", record.당일실현손익);
                    cmd.Parameters.AddWithValue("@보유종목수", record.보유종목수);
                    cmd.Parameters.AddWithValue("@당일매수건수", record.당일매수건수);
                    cmd.Parameters.AddWithValue("@당일매도건수", record.당일매도건수);
                    cmd.Parameters.AddWithValue("@당일매도승수", record.당일매도승수);
                    cmd.Parameters.AddWithValue("@당일매도패수", record.당일매도패수);
                    cmd.ExecuteNonQuery();
                }
            }
        }
        ...
        /// <summary>
        /// 일별 자산 추이 전체 조회 (날짜 오름차순)
        /// </summary>
        public List<DailyAssetRecord> LoadDailyAssets()
        {
            var list = new List<DailyAssetRecord>();
            using (var conn = new SQLiteConnection(_dbPath))
            {
                conn.Open();
                string sql = "SELECT * FROM DailyAssetTable ORDER BY 날짜 ASC";
                using (var cmd = new SQLiteCommand(sql, conn))
                using (var reader = cmd.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        list.Add(new DailyAssetRecord
                        {
                            날짜 = reader["날짜"]?.ToString() ?? "",
                            추정예탁자산 = long.TryParse(reader["추정예탁자산"]?.ToString(), out long a) ? a : 0,
                            총매입금액 = long.TryParse(reader["총매입금액"]?.ToString(), out long b) ? b : 0,
                            총평가금액 = long.TryParse(reader["총평가금액"]?.ToString(), out long c) ? c : 0,
                            보유평가손익 = long.TryParse(reader["보유평가손익"]?.ToString(), out long d) ? d : 0,
                            당일실현손익 = long.TryParse(reader["당일실현손익"]?.ToString(), out long e) ? e : 0,
                            보유종목수 = int.TryParse(reader["보유종목수"]?.ToString(), out int f) ? f : 0,
                            당일매수건수 = int.TryParse(reader["당일매수건수"]?.ToString(), out int g) ? g : 0,
                            당일매도건수 = int.TryParse(reader["당일매도건수"]?.ToString(), out int h) ? h : 0,
                            당일매도승수 = int.TryParse(reader["당일매도승수"]?.ToString(), out int i) ? i : 0,
                            당일매도패수 = int.TryParse(reader["당일매도패수"]?.ToString(), out int j) ? j : 0,
                        });
                    }
                }
            }
