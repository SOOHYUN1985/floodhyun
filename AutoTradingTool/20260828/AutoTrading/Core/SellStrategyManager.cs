...
namespace AutoTradingTest.Core
{
    ...
    public class SellStrategyManager
    {
        ...
        public void UpdateTrailingStop(DBInfo holding, int currentPrice)
        {
            ...
            // 로스컷가격 미설정(레거시 데이터/이전 버그 잔존) 시 매수가 -R%로 자동 초기화
            if (holding.로스컷가격 <= 0 && holding.로스컷단계 == 0)
                holding.로스컷가격 = (int)(holding.매수가격 * (1.0 - R / 100.0));
        ...
        {
            if (!_config.부분익절활성화) return null; // 비활성화 시 스킵
        ...
        /// (장대'음'봉의 본질: 종가 < 시가가 전제 — 갭상승 후 약보합 마감은 제외)
        /// </summary>
        private SellSignal Check장대음봉(DBInfo holding, RealTimePrice price, long 최근50일최대거래량)
        {
            ...
            // 음봉 조건: 시가 > 종가 (시가 미수신 시 전일종가 대비 하락으로 대체)
            bool bearishCondition = price.시가 > 0
                ? price.현재가 < price.시가
                : price.현재가 < price.전일종가;

            if (volumeCondition && candleCondition && bearishCondition)
            {
                return new SellSignal
                {
                    종목코드 = holding.종목코드,
        ...
        /// <summary>
        /// 5. 최대보유일 초과: 보유일 > 최대보유일 AND 평균손익 기준 손실 중
        /// 부분매도(nR절반익절/EMA이탈) 실현수익 + 잔여수량 평가손익을 합산해 판단
        /// </summary>
        private SellSignal Check최대보유일(DBInfo holding, RealTimePrice price)
        {
            if (holding.보유일 <= _config.최대보유일) return null;
            if (holding.매수가격 <= 0 || holding.매수수량 <= 0) return null;

            // 부분매도 실현 수익 + 잔여수량 평가 수익 합산
            long 실현수익 = 0;
            if (holding.nR절반매도 && holding.nR절반매도수량 > 0 && holding.nR절반매도가격 > 0)
                실현수익 += (long)(holding.nR절반매도가격 - holding.매수가격) * holding.nR절반매도수량;
            if (holding.이평매도수량 > 0 && holding.이평매도가격 > 0)
                실현수익 += (long)(holding.이평매도가격 - holding.매수가격) * holding.이평매도수량;
            long 평가수익 = (long)(price.현재가 - holding.매수가격) * holding.보유수량;
            long 총손익 = 실현수익 + 평가수익;

            if (총손익 < 0) // 평균 기준 손실 상태
            {
                return new SellSignal
                {
                    종목코드 = holding.종목코드,
                    ...
                    매도유형 = SellType.최대보유일,
                    매도수량 = holding.보유수량,
                    매도이유 = $"보유 {holding.보유일}일 초과 + 평균손실 {총손익:N0}원",
                    전량매도 = true
                };
            }
            return null;
        ...
        private SellSignal CheckEMA이탈(DBInfo holding, RealTimePrice price, Dictionary<int, int> emaValues)
        {
            ...
            foreach (int period in remainingStages)
            {
                ...
                {
                    // EMA전량매도 옵션: true면 첫 EMA 이탈 시 전량매도
                    if (_config.EMA전량매도)
                    {
                        return new SellSignal
                        {
                            ...
                            매도유형 = SellType.이평이탈,
                            매도수량 = holding.보유수량,
                            매도이유 = $"EMA{period} 이탈 전량매도 (현재가 {price.현재가:N0} < EMA{period} {emaValue:N0})",
                            전량매도 = true
                        };
                    }
