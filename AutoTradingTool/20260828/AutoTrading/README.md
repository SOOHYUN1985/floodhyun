...
> **현재 상태**: 모의투자 운영 중 (2026-06-01)  
> 자동매수 + 자동매도 + 실시간 감시 + 일일 리포트 + 성과 차트 + 거래 이력 조회까지 구현 완료
...
├── PerformanceChartForm.cs   # 계좌 수익률 vs 주요 지수 비교 차트 창
├── TradeHistoryForm.cs       # 거래 이력 조회 창 (기간/전략/손익 필터)
├── Program.cs                # 엔트리포인트
├── App.config                # 앱 설정
...
│   ├── DbManager.cs          # SQLite CRUD (보유종목/거래내역/일별자산)
│   ├── LogManager.cs         # 파일 로깅 (비동기 큐 + 500ms 일괄 flush)
...
| 파라미터 | 기본값 | 설명 |
|----------|--------|------|
| R값 | 7.0 | 기본 손절률 (%) — 로스컷/트레일링 단위 |
| N배수 | 3 | 절반 익절 기준 (수익률 ≥ R×N% 시 발동) |
| EMA매도기간 | [5,10,20] | 단계별 EMA 이탈 매도 기간 리스트 |
| EMA전량매도 | false | true 시 마지막 EMA 이탈 단계에서 전량매도 |
| 부분익절활성화 | true | false 시 nR 절반 익절 규칙 비활성화 |
| 최대보유일 | 20 | 보유일 초과 + 손실 시 전량매도 |
| 종목당최대투자금 | 20,000,000 | 1종목당 최대 매수금액 (원) |
| 최대보유종목수 | 30 | 동시 보유 가능 최대 종목 수 |
| 슬리피지매수/매도 | 0.3 | 주문가격 슬리피지 (%) |
| 시가총액하한 | 500,000,000,000 | 매수 가능 시가총액 하한 (5000억) |
| 재매수금지기간 | 0 | 매도 후 재매수 금지일 (0=비활성) |
| 급락매도기준 | 86.5 | 전일종가 대비 하한 비율 (%) — 86.5 = -13.5% |
| 장대음봉거래량비율 | 0.618 | 장대음봉 조건: 당일거래량 ≥ 50일최대 × 비율 |
| 장대음봉스프레드기준 | 15 | 장대음봉 조건: (고가-종가)/전일종가 × 100 ≥ 기준 |
...
- **성과 차트** (`PerformanceChartForm`): 내 계좌 수익률 vs 코스피/코스닥/나스닥/S&P500/다우/러셀2000 비교 (Yahoo Finance 데이터, 기간 선택 가능)
- **거래 이력 조회** (`TradeHistoryForm`): 기간/전략명/손익 필터 + 승률/평균보유일/평균수익률 요약 카드
...
- **파일 로그**: `bin/Release/Log/YYYY-MM-DD.txt` (일자별 자동 생성, 타임스탬프 밀리초 포함)
- **UI 로그**: 하단 로그 패널 (다크 테마, 타임스탬프 포함)
- **설정 로그**: 앱 시작 시 로드된 전략 파라미터 출력
- **일일 리포트**: 장 마감 시 매수/매도 요약 + 보유현황 스냅샷

> LogManager는 `ConcurrentQueue` + 500ms 일괄 flush 방식으로 UI 스레드 블로킹 없이 비동기 기록합니다.
...
| Phase 14 | 최종 안정화 (비동기 로그, 레이스컨디션) | ✅ 완료 |
