...
def generate_report(kospi_results, kosdaq_results, futures_results,
    ...
    lines.append("")
    lines.append("| 매도 강도 | 기준 (억원) | 발생횟수 | D+0 | D+5 | D+10 | D+20 | D+30 | 3개월 | 6개월 | 1년 |")
    lines.append("|:--------:|:----------:|:------:|:---:|:---:|:----:|:----:|:----:|:----:|:----:|:---:|")
    ...
    for r in kospi_results:
        ...
        lines.append(
            ...
            f"{vals['D+20']} | {vals['D+30']} | {vals['3개월']} | {vals['6개월']} | {vals['1년']} |"
        )
...
def main():
    ...
    all_lines.append("")
    if short_best:
        all_lines.append("| 순위 | 매도 시점 | 평균 수익률 | 상승확률 | 효율점수 |")
        all_lines.append("|:----:|:--------:|:----------:|:-------:|:-------:|")
        for i, (d, avg, prob, score, n) in enumerate(short_best[:5]):
            all_lines.append(f"| {i+1} | D+{d} | {avg:+.2f}% | {prob:.0f}% | {score:.2f} |")
        all_lines.append("")
    ...
    if long_best:
        all_lines.append("| 순위 | 매도 시점 | 평균 수익률 | 상승확률 | 효율점수 |")
        all_lines.append("|:----:|:--------:|:----------:|:-------:|:-------:|")
        for d, avg, prob, score, n in long_best[:5]:
            for name, days in LONG_PERIODS.items():
                if days == d:
                    label = name
                    break
                ...
                label = f"D+{d}"
            all_lines.append(f"| - | {label} (D+{d}) | {avg:+.2f}% | {prob:.0f}% | {score:.2f} |")
        all_lines.append("")
