...
│   ├── crash_buy_strategy.py       # MDD 급락 매수 전략 백테스트 (다단계 진입, 6청산 모드)
...
│
├─ results/
│   ├── analysis/                   # 심층 분석 리포트 + 차트 이미지
│   └── reports/
│       ├── backtest/               # 일일 고점판독 리포트 + 밸류에이션 차트
...
[4] 보통주/우선주 괴리율 (27쌍)
[5] 48시간 지난 리포트 자동정리 → 최신 리포트 자동오픈
```
...
python crash_buy_strategy.py                # MDD 급락매수 백테스트
```
