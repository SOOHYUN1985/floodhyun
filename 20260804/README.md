...
실행 순서:
  ...
  [7]    외국인 순매수 심층 분석
  [8]    담보비율 시뮬레이션
  [9]    반도체 밸류에이션 (삼성전자/SK하이닉스)
  [10]   추세/MDD 차트
  [11]   시장 전략 매매 타이밍 리포트
  [12]   코스피 MDD 심층 분석 리포트
  [13]   삼성전자·SK하이닉스 컨센서스 리포트
  [14]   최신 리포트 자동 열기
...
**`stock_analysis.bat` 더블클릭** 또는 CLI 실행

```
입력: 종목명 또는 종목코드 (6자리)
예)  stock_analysis.bat 현대차
     stock_analysis.bat 005380

→ 결과: results/weekly_research/{종목명}_목표주가분석_{날짜}.md
```
...
python stock_consensus_report.py [종목명]  # 개별 종목 목표주가 컨센서스
python micron_target_report.py            # 마이크론 EPS 기반 목표주가 산출

# ── 시장 전략 ──
python market_strategy_report.py          # 매매 타이밍 + 피보나치 전략
python mdd_analysis_report.py             # MDD 심층 분석 + 추가매수 단계
...
│   ├── trough_detector.py      저점 판독 백테스터 (PeakDetector 거울, 매수 신호)
...
│   ├── risk_metrics.py         Sharpe/Sortino/Calmar/Monte Carlo/Kelly/ATR
...
│   ├── stock_valuation_report.py        반도체 밸류에이션 (삼성전자/SK하이닉스)
│   ├── semiconductor_consensus_report.py 삼성전자·SK하이닉스 증권사 컨센서스
│   ├── stock_consensus_report.py        개별 종목 목표주가 컨센서스
│   ├── micron_target_report.py          마이크론 EPS 기반 목표주가 산출
│   ├── market_strategy_report.py        매매 타이밍 + 피보나치 전략 리포트
│   └── mdd_analysis_report.py           MDD 심층 분석 + 추가매수 단계 전략
│
├── 데이터 수집 ─────────────────────────────────────────────
│   └── update_market_data.py   일일 업데이트 (지수+54종목, 네이버 fallback)
│
├── 자동화 배치 ─────────────────────────────────────────────
│   ├── daily_backtest.bat      매일: DB→밸류에이션→백테스트→괴리율 (5단계)
│   ├── daily_position.bat      매일 (장마감 후): 포지션 분석
│   ├── weekly_research.bat     주 1회: 계절성→외국인→담보→밸류에이션 (14단계)
│   ├── stock_analysis.bat      수시: 개별 종목 목표주가 조회
