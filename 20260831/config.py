...
# 백테스트 파라미터
# 인접 임계값의 중복 신호를 줄이고, 서로 다른 시장 국면을 대표하는 값만 사용합니다.
BACKTEST_PARAMS = {
    ...
    # 이동평균 기간 (일) - 단기/중기/장기 대표 시간축
    'ma_min': 5,
    'ma_max': 120,
    'ma_step': 5,
    'ma_periods': [5, 10, 20, 40, 60, 120],

    # 기술적 지표 임계값 - 각 과열 단계를 대표하는 값만 사용
    'rsi_thresholds': [60, 70, 80, 90],
    'stoch_thresholds': [65, 75, 85, 95],
    'mfi_thresholds': [65, 75, 85, 90],
    'cci_thresholds': [80, 120, 160, 200, 250],
    'bb_ratios': [1.01, 1.03, 1.06, 1.10, 1.15],
    
    # 새로 추가된 지표 임계값
    'obv_ma_cross': True,
    'dmi_thresholds': [15, 25, 35, 45],
    'volume_ratio_thresholds': [1.3, 1.8, 2.5, 3.5],
    'vwap_ratios': [1.01, 1.03, 1.06, 1.10],
    
    # 하락반전 전략용 파라미터
    'reversal_rsi_thresholds': [60, 70, 80, 90],
    'reversal_stoch_thresholds': [65, 75, 85, 95],
    'reversal_mfi_thresholds': [65, 75, 85],
    'reversal_cci_thresholds': [50, 100, 150, 200, 300],
    'reversal_bb_ratios': [0.95, 1.00, 1.05, 1.10, 1.15],
    'reversal_disparity_thresholds': [102, 108, 115, 122, 128],
    'reversal_adx_thresholds': [15, 25, 35, 45],
    'reversal_macd_periods': [8, 12, 16, 20],
    
    # 하락반전용 새로운 지표
    'reversal_dmi_thresholds': [15, 25, 35],

    # 최소 확인 신호 수 (통계적 신뢰도: 8개 이상 권장)
    'min_signals': 7,             # 5→7: 과적합 방지 강화
    'min_signals_regime': 3,      # 현재 레짐별 최소 신호 수

    # 외국인 매매 임계값
    'foreign_buy_pct': [5, 10],   # 상위 5%, 10%
    'foreign_sell_pct': [5, 10],  # 하위 5%, 10%
    
    # MDD 조건 임계값
    'mdd_severe_pct': -20,        # -20% 이하 = 심각한 하락
    'mdd_extreme_pct': -30,       # -30% 이하 = 극단적 하락
    ...
    # 후보 전략 중복 제거: 신호일 중복률이 85% 이상이면 품질이 높은 하나만 유지
    'candidate_signal_similarity': 0.85,
...
# ──────────────────────────────────────────────────────────
CURRENT_FWD_EPS = 1100     # ← 오늘자 12M Forward EPS (수시 업데이트)
