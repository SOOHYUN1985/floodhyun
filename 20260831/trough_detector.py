...
class TroughDetector(PeakDetector):
    ...
    def run_backtest(self):
        ...
        print("\n3️⃣ 외국인 매매 + MDD 복합 매수 전략...")
        self._test_foreign_flow_buy_strategies()
    ...
    def _test_foreign_flow_buy_strategies(self):
        """
        외국인 매매동향 + MDD 기반 매수 전략
        - 외국인 대량 매수 + 기술적 과매도
        - MDD 심화 구간 + 외국인 매수 전환
        - RSI 모멘텀 반전 + MDD 구간
        """
        has_foreign = 'foreign_buy5_sig' in self.df.columns
        has_mdd     = 'rolling_mdd' in self.df.columns
        has_mom     = 'rsi_mom_up' in self.df.columns
        ...
        # ── 외국인 매매 기반 전략 ──────────────────────────
        if has_foreign:
            buy5  = self.df['foreign_buy5_sig']  == 1
            buy10 = self.df['foreign_buy10_sig'] == 1
            turn  = self.df['foreign_turn_buy']  == 1

            # 1. 외국인 강한 매수(상위 5%) + 기술적 과매도
            for rsi_th in [25, 30, 35, 40]:
                sig = buy5 & (self.df['RSI'] <= rsi_th)
                self._evaluate_strategy(f"외국인상위5%매수 + RSI{rsi_th}-", sig, 'reversal_up', None, None)
                test_count += 1

            for mfi_th in [20, 25, 30, 35]:
                sig = buy5 & (self.df['MFI'] <= mfi_th)
                self._evaluate_strategy(f"외국인상위5%매수 + MFI{mfi_th}-", sig, 'reversal_up', None, None)
                test_count += 1

            # 외국인 상위 5% + Stoch 과매도
            for stoch_th in [10, 15, 20]:
                sig = buy5 & (self.df['Stoch_K'] <= stoch_th)
                self._evaluate_strategy(f"외국인상위5%매수 + Stoch{stoch_th}-", sig, 'reversal_up', None, None)
                test_count += 1
            ...
            sig = buy10 & macd_gold
            self._evaluate_strategy("외국인상위10%매수 + MACD골든", sig, 'reversal_up', None, None)
            test_count += 1

            # 2. 외국인 매수 전환 (매도→매수 전환) + 과매도
            for rsi_th in [30, 35, 40, 45]:
                sig = turn & (self.df['RSI'] <= rsi_th)
                self._evaluate_strategy(f"외국인매수전환 + RSI{rsi_th}-", sig, 'reversal_up', None, None)
                test_count += 1

            for mfi_th in [25, 30, 35]:
                sig = turn & (self.df['MFI'] <= mfi_th)
                self._evaluate_strategy(f"외국인매수전환 + MFI{mfi_th}-", sig, 'reversal_up', None, None)
                test_count += 1

            # 외국인 전환 + Stoch + RSI
            for rsi_th in [35, 40]:
                for stoch_th in [15, 20]:
                    sig = turn & (self.df['RSI'] <= rsi_th) & (self.df['Stoch_K'] <= stoch_th)
                    self._evaluate_strategy(
                        f"외국인매수전환 + RSI{rsi_th}- + Stoch{stoch_th}-",
                        sig, 'reversal_up', None, None
                    )
                    test_count += 1

            # 3. 외국인 5일 누적 강한 매수 + 이격도 하향
            if 'foreign_5d_cum' in self.df.columns:
                pct90 = self.df['foreign_5d_cum'].dropna().quantile(0.90)
                if not pd.isna(pct90) and pct90 > 0:
                    strong5 = self.df['foreign_5d_cum'] >= pct90
                    for ma in [10, 20, 60]:
                        disp_col = f'Disparity_{ma}'
                        if disp_col not in self.df.columns:
                            continue
                        for disp_th in [88, 91, 94]:
                            sig = strong5 & (self.df[disp_col] <= disp_th)
                            self._evaluate_strategy(
                                f"외국인5일강매수90% + 이격도{disp_th}(MA{ma})-",
                                sig, 'reversal_up', disp_th, ma
                            )
                            test_count += 1
        ...
        if has_mdd:
            ...
            extreme = self.df['mdd_extreme']  # -30% 이하
            crash   = self.df['mdd_crash']    # -40% 이하

            # 4. MDD -20%~-30% 구간 + 기술적 과매도 (역사적 좋은 매수 시점)
            for rsi_th in [25, 30, 35]:
                sig = severe & (self.df['RSI'] <= rsi_th)
                self._evaluate_strategy(f"MDD-20%이하 + RSI{rsi_th}-", sig, 'reversal_up', None, None)
                test_count += 1

            for mfi_th in [20, 25, 30]:
                sig = severe & (self.df['MFI'] <= mfi_th)
                self._evaluate_strategy(f"MDD-20%이하 + MFI{mfi_th}-", sig, 'reversal_up', None, None)
                test_count += 1

            # MDD -20% + ADX 강한 하락추세 + RSI (추세 중 과매도)
            for adx_th in [35, 40, 45]:
                for rsi_th in [25, 30]:
                    sig = severe & (self.df['ADX'] >= adx_th) & (self.df['RSI'] <= rsi_th)
                    self._evaluate_strategy(
                        f"MDD-20%이하 + ADX{adx_th}+ + RSI{rsi_th}-",
                        sig, 'reversal_up', None, None
                    )
                    test_count += 1

            # 5. MDD -30% 이하 = 역사적 극단 저점
            for rsi_th in [20, 25, 30]:
                sig = extreme & (self.df['RSI'] <= rsi_th)
                self._evaluate_strategy(f"MDD-30%이하 + RSI{rsi_th}-", sig, 'reversal_up', None, None)
                test_count += 1

            if has_mom:
                sig = extreme & self.df['rsi_mom_up']
                self._evaluate_strategy("MDD-30%이하 + RSI모멘텀반전", sig, 'reversal_up', None, None)
                test_count += 1

                if 'macd_hist_improving' in self.df.columns:
                    sig = extreme & self.df['rsi_mom_up'] & self.df['macd_hist_improving']
                    self._evaluate_strategy(
                        "MDD-30%이하 + RSI모멘텀반전 + MACD히스토개선",
                        sig, 'reversal_up', None, None
                    )
                    test_count += 1

            # MDD -40% 이하 (폭락 구간) = 무조건 역발상 매수 구간
            for rsi_th in [20, 25, 30, 35]:
                sig = crash & (self.df['RSI'] <= rsi_th)
                self._evaluate_strategy(f"MDD-40%이하(폭락) + RSI{rsi_th}-", sig, 'reversal_up', None, None)
                test_count += 1

        # ── 외국인 + MDD 복합 (가장 강력한 신호) ─────────
        if has_foreign and has_mdd:
            buy10 = self.df['foreign_buy10_sig'] == 1
            buy5  = self.df['foreign_buy5_sig']  == 1
            turn  = self.df['foreign_turn_buy']  == 1
            severe  = self.df['mdd_severe']
            extreme = self.df['mdd_extreme']

            # 6. MDD -20% + 외국인 상위 10% 매수 + RSI 과매도
            for rsi_th in [30, 35, 40]:
                sig = severe & buy10 & (self.df['RSI'] <= rsi_th)
                self._evaluate_strategy(
                    f"MDD-20%이하 + 외국인상위10% + RSI{rsi_th}-",
                    sig, 'reversal_up', None, None
                )
                test_count += 1

            # 7. MDD -20% + 외국인 매수 전환 (핵심 복합 신호)
            for rsi_th in [30, 35, 40]:
                sig = severe & turn & (self.df['RSI'] <= rsi_th)
                self._evaluate_strategy(
                    f"MDD-20%이하 + 외국인매수전환 + RSI{rsi_th}-",
                    sig, 'reversal_up', None, None
                )
                test_count += 1

            sig = severe & turn
            self._evaluate_strategy("MDD-20%이하 + 외국인매수전환", sig, 'reversal_up', None, None)
            test_count += 1

            # 8. MDD -30% + 외국인 상위 5% = 역사적 최강 신호
            for rsi_th in [25, 30, 35]:
                sig = extreme & buy5 & (self.df['RSI'] <= rsi_th)
                self._evaluate_strategy(
                    f"MDD-30%이하 + 외국인상위5% + RSI{rsi_th}-",
                    sig, 'reversal_up', None, None
                )
                test_count += 1

            sig = extreme & buy5
            self._evaluate_strategy("MDD-30%이하 + 외국인상위5%매수", sig, 'reversal_up', None, None)
            test_count += 1
        ...
        if has_mom:
            ...
            for ma in [10, 20, 60]:
                ...
                if disp_col not in self.df.columns:
                    continue
                for disp_th in [88, 91, 94]:
                    sig = self.df['rsi_mom_up'] & (self.df[disp_col] <= disp_th)
                    self._evaluate_strategy(
                        f"RSI모멘텀반전 + 이격도{disp_th}(MA{ma})-",
                        sig, 'reversal_up', disp_th, ma
                    )
                    test_count += 1

            if 'macd_hist_improving' in self.df.columns:
                # 10. MACD 히스토그램 2일 연속 개선 + RSI 과매도
                for rsi_th in [25, 30, 35]:
                    sig = self.df['macd_hist_improving'] & (self.df['RSI'] <= rsi_th)
                    self._evaluate_strategy(
                        f"MACD히스토2일개선 + RSI{rsi_th}-",
                        sig, 'reversal_up', None, None
                    )
                    test_count += 1

        new_found = len(self.strategies) - found_before
        print(f"   ✅ 외국인+MDD 전략 테스트 완료: {test_count}회, 발견: {new_found}개")
