...
echo [3/5] 코스피 + 코스닥 백테스트 및 1일~6개월 전망 분석 중...
echo ----------------------------------------------------------------------
"%PYTHON%" main.py --all
if %errorlevel% neq 0 (
    echo [ERROR] 백테스트 실패
...
echo [5/5] 리포트 열기...
echo ----------------------------------------------------------------------
"%PYTHON%" -c "import glob,os;files=sorted(glob.glob('results/daily_backtest/핵심브리핑_*.md'),key=os.path.getmtime);[print(f'  ⚡ {os.path.basename(f)}') or os.startfile(f) for f in files[-1:]] if files else print('  [WARNING] 브리핑 없음')"
