...
class DataLoader:
    ...
    def calculate_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # ── Rolling MDD (250일 고점 대비 낙폭) ──
        roll_max = df['close'].rolling(250, min_periods=30).max()
        df['rolling_mdd'] = (df['close'] / roll_max - 1) * 100   # 음수 (%)
        df['mdd_severe']  = df['rolling_mdd'] <= -20   # -20% 이하
        df['mdd_extreme'] = df['rolling_mdd'] <= -30   # -30% 이하
        df['mdd_crash']   = df['rolling_mdd'] <= -40   # -40% 이하

        # ── 모멘텀 반전 신호 (저점에서 방향 전환 감지) ──
        # RSI 상승 반전: 과매도 상태에서 RSI가 다시 오르기 시작
        df['rsi_mom_up'] = (
            (df['RSI'] < 35) &
            (df['RSI'] > df['RSI'].shift(1)) &
            (df['RSI'].shift(1) <= df['RSI'].shift(2))
        )
        # MACD 히스토그램 2일 연속 개선 (음수지만 줄어드는 중)
        df['macd_hist_improving'] = (
            (df['MACD_Hist'] < 0) &
    ...
    def merge_investor_flow(self, df: pd.DataFrame, market: str) -> pd.DataFrame:
        """
        외국인 매매동향 데이터를 가격 데이터프레임에 병합
        market: 'kospi' → DB market='01', 'kosdaq' → '02'
        """
        investor_db = os.path.join(os.path.dirname(self.db_path), 'investor_data.db')
        if not os.path.exists(investor_db):
            logger.debug("investor_data.db 없음 — 외국인 매매 신호 생략")
            return df

        market_code = '01' if market.lower() == 'kospi' else '02'
        try:
            conn = sqlite3.connect(investor_db)
            inv = pd.read_sql_query(
                "SELECT date, foreign_ AS foreign_net FROM investor_daily "
                "WHERE market=? ORDER BY date",
                conn, params=(market_code,), parse_dates=['date']
            )
            conn.close()
        except Exception as e:
            logger.warning(f"외국인 데이터 로드 실패: {e}")
            return df
        ...
        df = df.copy()
        df['foreign_net'] = inv['foreign_net'].reindex(df.index).fillna(0)

        # 롤링 250일 퍼센타일 (0=극단 매도, 100=극단 매수)
        df['foreign_roll_pct'] = df['foreign_net'].rolling(250, min_periods=30).apply(
            lambda x: (x[:-1] < x[-1]).sum() / max(len(x) - 1, 1) * 100, raw=True
        )

        # 이진 신호
        df['foreign_buy5_sig']  = (df['foreign_roll_pct'] >= 95).astype(int)  # 상위 5%
        df['foreign_sell5_sig'] = (df['foreign_roll_pct'] <= 5).astype(int)   # 하위 5%
        df['foreign_buy10_sig'] = (df['foreign_roll_pct'] >= 90).astype(int)  # 상위 10%
        df['foreign_sell10_sig']= (df['foreign_roll_pct'] <= 10).astype(int)  # 하위 10%

        # 누적 순매수 (5/20일)
        df['foreign_5d_cum']  = df['foreign_net'].rolling(5).sum()
        df['foreign_20d_cum'] = df['foreign_net'].rolling(20).sum()

        # 매매 방향 전환
        df['foreign_turn_buy']  = (
            (df['foreign_net'] > 0) & (df['foreign_net'].shift(1) < 0)
        ).astype(int)
        df['foreign_turn_sell'] = (
            (df['foreign_net'] < 0) & (df['foreign_net'].shift(1) > 0)
        ).astype(int)

        logger.info(f"외국인 매매 병합 완료 ({market.upper()}): "
                    f"{inv.index[0].date()} ~ {inv.index[-1].date()}, {len(inv)}일")
        return df
