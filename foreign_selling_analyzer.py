...
def generate_report(all_results):
    ...
    for period_group in ["20년", "10년"]:
        ...
        for r in results_in_group:
            ...
            for idx_name, ret_df in [("코스피", r["kospi_returns"]), ("코스닥", r["kosdaq_returns"])]:
                ...
                lines.append(f"| {name}→{idx_name} | 평균 수익률 | {d1_s} | {d5_s} | {d10_s} | {d20_s} | {d30_s} | {up_s} |")
    ...
    for r in all_results:
        ...
        for idx_name, ret_df in [("코스피", r["kospi_returns"]), ("코스닥", r["kosdaq_returns"])]:
            ...
            # 개별 이벤트별 수익률
            lines.append(f"| 날짜 | D+1 | D+3 | D+5 | D+10 | D+15 | D+20 | D+30 |")
            lines.append("|:----:|:---:|:---:|:---:|:----:|:----:|:----:|:----:|")
    ...
    lines.append("## 💡 투자 시사점")
    lines.append("")
    lines.append("### 외국인 대량 순매도 이후 패턴")
    lines.append("")
    
    # 20년 결과에서 패턴 추출
    for r in all_results:
        if "20년" in r["period_name"]:
            kr = r["kospi_returns"]
            if not kr.empty and "D+20" in kr:
                ...
                lines.append(f"- **{r['market_name']}** 순매도 극단일 이후 20거래일: "
                           f"코스피 평균 {avg_d20:+.2f}% ({direction}), "
                           f"상승확률 {up_prob:.0f}%")
    
    lines.append("")
    lines.append("### 주의사항")
    lines.append("")
    lines.append("1. 외국인 순매도 극단값은 시장 공포 극대화 시점과 일치하는 경향")
    lines.append("2. 단기(D+1~5)에는 추가 하락 가능성 주의")
    lines.append("3. 과거 패턴이 미래를 보장하지 않음")
    lines.append("4. 선물 매도는 현물 시장과 연동되나 헤지 목적일 수 있음")
    lines.append("5. 순매도 금액 단위: 억원")
    lines.append("")
    lines.append("---")
    lines.append("")
