...
class StrategySelector:
    ...
    def _composite_score(self, strategy: Dict) -> float:
        ...
        구성:
          - 승률 (30% 가중)
          - Profit Factor (20% 가중, capped at 5)
          - 현재 레짐 승률 (15% 가중)
          - 신호 수 보너스 (10% 가중, 많을수록 통계적 유의)
          - MAE 패널티 (15% 가중, 틀렸을 때 손실이 작을수록 좋음)
          - OOS 승률 보너스 (10% 가중, Walk-Forward 검증 결과)
        """
        ...
        # MAE 패널티 (틀렸을 때 최대 역행 — 작을수록 좋음)
        max_adverse = strategy.get('max_adverse', 10)
        # max_adverse가 0이면 100점, 20%이면 0점
        mae_score = max(0, 100 - (max_adverse * 5))
        
        # OOS 승률 보너스 (Walk-Forward 검증 결과)
        oos_wr = strategy.get('oos_win_rate')
        if oos_wr is not None:
            oos_score = oos_wr  # 0~100 스케일
        else:
            oos_score = win_rate * 0.7  # OOS 없으면 IS의 70%로 보수적 추정
