...
echo [4/10] 외국인 순매도 Top20 분석 (네이버 스크래핑)...
echo ----------------------------------------------------------------------
...
echo [10/12] 코스피/코스닥 추세 + MDD 차트 생성...
echo ----------------------------------------------------------------------
...
echo [12/12] 리포트 열기...
echo ----------------------------------------------------------------------
"%PYTHON%" -c "import glob,os;patterns=['results/weekly_research/명절효과_분석_*.md','results/weekly_research/연말연초_효과_분석_*.md','results/weekly_research/외국인_순매도_심층분석_*.md','results/weekly_research/외국인_순매수_심층분석_*.md','results/weekly_research/담보대출_전략_*.md','results/weekly_research/반도체_밸류에이션_분석_*.md','results/weekly_research/시장전략_매매타이밍_*.md'];[print(f'  {os.path.basename(f)}') or os.startfile(f) for p in patterns for f in sorted(glob.glob(p),key=os.path.getmtime)[-1:]]"
