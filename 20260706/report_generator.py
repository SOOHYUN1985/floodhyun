...
"""
    ...
    def _generate_strategy_list_table(self) -> str:
        """전략 목록 표 생성 (OOS 승률, MAE, Sharpe, Kelly 포함)"""
        rows = []
        for i, s in enumerate(self.strategies, 1):
            ...
            # Kelly (Half-Kelly 권장 — 보수적)
            half_k = s.get('half_kelly', 0)
            kelly_str = f"{half_k * 100:.0f}%"
            
            rows.append(
                f"| {i} | {s['name']} | {type_name} | {win_rate:.1f}% | {pf_str} | "
                f"{sharpe_str} | {kelly_str} | {avg_ret:+.1f}% | {s['signal_count']} | "
                f"{oos_str} | {mae_str} |"
            )
        return "\n".join(rows)
    ...
    def _generate_validation_summary(self) -> str:
        ...
        if wf_strategies:
            text += "**Walk-Forward 검증 (OOS = Out-of-Sample)**\n\n"
            text += "| 전략 | IS 승률 | OOS 승률 | 하락폭 | 검증 |\n"
            text += "|------|:-------:|:--------:|:------:|:----:|\n"
            for s in wf_strategies:
                ...
                text += f"| {s['name']} | {is_wr:.0f}% | {oos_wr:.0f}% | {degrad:+.0f}%p | {validated} |\n"
