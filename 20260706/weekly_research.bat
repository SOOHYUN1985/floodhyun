...
echo [14/14] 리포트 열기...
echo ----------------------------------------------------------------------
"%PYTHON%" -c "import glob,os;patterns=['results/weekly_research/명절효과_분석_*.md','results/weekly_research/연말연초_효과_분석_*.md','results/weekly_research/외국인_순매도_심층분석_*.md','results/weekly_research/외국인_순매수_심층분석_*.md','results/weekly_research/담보대출_전략_*.md','results/weekly_research/반도체_밸류에이션_분석_*.md','results/weekly_research/시장전략_매매타이밍_*.md','results/weekly_research/코스피_MDD_심층분석_리포트_*.md','results/weekly_research/삼전하닉목표주가_by_Analist_*.md'];[print(f'  {os.path.basename(f)}') or os.startfile(f) for p in patterns for f in sorted(glob.glob(p),key=os.path.getmtime)[-1:]]"
