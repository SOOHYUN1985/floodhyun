...
def generate_report(name: str, code: str,
    ...
    with open(filename, 'w', encoding='utf-8') as f:
        ...
        f.write('## 매수 원칙\n\n')
        f.write('* 목표가의 **70% 이하** → **강력매수**\n')
        f.write('* 목표가의 **70~80%** → **매수**\n')
        f.write('* 목표가의 **80% 이상** → **신규매수 중단**\n\n')
        f.write('---\n\n')
        ...
        # ── 매수 기준 적용 ─────────────────────────────────────────────
        f.write('## 매수 기준 적용\n\n')
        f.write('| 기준 | 목표가 | 강력매수 (70%) | 매수 (80%) | 현재가/목표가 | 판단 |\n')
        f.write('|:-----|------:|----------:|-------:|------:|:----|\n')
        ...
        for label, tp in criteria:
            if tp and price:
                ratio = price / tp
                v     = _verdict(price, tp)
                f.write(f'| {label} | {tp:,} | {int(tp * 0.7):,} | {int(tp * 0.8):,} | {ratio:.1%} | {v} |\n')
